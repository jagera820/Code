...
def main():
    """Use this script to automate tracking sheet creation for
    infra projects (ex. deprecating a field or explore).
    The script compares the output of content validator in production
    and development mode. Additional broken content in development mode
    will be outputted to a csv file. The script also returns details
    about the errors and the number of dashboard runs & last run date."""
    ...
    if new_broken_content:
        ...
        num_rows = merged['unique_id'].count()
        num_content = merged.groupby('content_type')['id'].nunique()
        print("Total broken content: {}".format(num_rows))
        print("Count of broken content by: {}".format(num_content))
    else:
...
def parse_broken_content(base_url, broken_content, folder_data):
    ...
    for item in broken_content:
        ...
        if content_type == "dashboard":
            creator = sdk.dashboard(id).user_name
        else:
            creator_id = sdk.look(id).user_id
            creator = sdk.user(creator_id).display_name
